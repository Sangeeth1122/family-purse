...
-- public.users.id has FK -> auth.users(id) (see 0001_init.sql). auth.users is
-- the GoTrue-managed table: a migration cannot insert into it, and on a fresh
-- real Supabase project the canonical demo ids do not exist there yet, so a
-- direct public.users insert violates the FK (SQLSTATE 23503). The demo data
-- therefore CANNOT be applied by `supabase db push`.
--
-- This file is BOTH the single canonical dataset AND a guard:
--   * it only seeds when the three canonical auth users already exist
--     (created via the admin API in scripts/seed-demo.ts) — so on a fresh real
--     Supabase project it applies as a clean no-op;
--   * it is idempotent — it never re-seeds over an existing canonical family.
--
-- Deployment on a real project:
--   1. supabase db push            (0001–0009; this file no-ops on a fresh DB)
--   2. npm run db:seed-demo        (creates the Auth users, then this file)
-- Local validation (npm run db:validate) supplies the auth ids in its stub, so
-- the canonical data still seeds there exactly as before.
...
begin
  if not exists (
     ...
     or exists (select 1 from public.families where id = '11111111-1111-4111-8111-111111111111') then
    return;
  end if;

  -- Users first (family_id defer), then the family, then link members back.
  -- on conflict: on a real project the auth-creation trigger already made
  -- these profiles, so the leading INSERT below is a no-op there and the NEXT
  -- statement settles the canonical role/names on those trigger-created rows.
  ...
  update public.users set
    role = case id
      when 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa1' then 'admin'::public.user_role
      else 'member'::public.user_role
    end,
    name = case id
      when 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa1' then 'Aravind'
      when 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa2' then 'Revathi'
      else 'Karthik'
    end
  where id in (
    'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa1',
